...
\\[\baselineskip]
At the moment there are four (4) different run options:
\begin{itemize}
\item {\color{green}\verb+calc-eom = T+}, {\color{green}\verb+calc-sus = T+}, {\color{red}\verb+QDataFile+ empty} $\rightarrow$ calculation on \verb+q-grid+
\item {\color{green}\verb+calc-eom = T+}, {\color{red}\verb+calc-sus = F+}, {\color{red}\verb+QDataFile+ empty} $\rightarrow$ calculation on \verb+q-grid+
\item {\color{red}\verb+calc-eom = F+}, {\color{green}\verb+calc-sus = T+}, {\color{red}\verb+QDataFile+ empty} $\rightarrow$ calculation on \verb+q-grid+
\item {\color{red}\verb+calc-eom = F+}, {\color{green}\verb+calc-sus = T+}, {\color{green}\verb+QDataFile+ \textbf{exists}} $\rightarrow$ calculation along these q-points.
\end{itemize}
