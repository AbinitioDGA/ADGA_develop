...
!=====================================================================================
   subroutine get_axes()
     ...
     integer(hsize_t),dimension(1) :: dim_iwb, dim_iwf, dim_iwf_max, dim_iwb_max
     integer :: err
     ...
     deallocate(iwf_array,iwb_array)
...
!===========================================================================================
   subroutine create_channels()
     ...
     call h5gcreate_f(new_file_id, "dens", grp_dens_id, err)
     call h5gcreate_f(new_file_id, "magn", grp_magn_id, err)
...
!=====================================================================================
    subroutine create_component(ichannel, iwb, ind_orb)
      implicit none
      ...
      call h5gcreate_f(new_file_id, grpname, grp_id, err)
      ...
      call h5dwrite_f(dset_id, type_r_id, tmp_r, dims, err)
      call h5dwrite_f(dset_id, type_i_id, tmp_i, dims, err)
      call h5dwrite_f(dset_err_id, h5t_native_double, tmp_err, dims, err)
...
!====================================================================================
   subroutine add_to_component(ichannel, iwb, ind_orb, g4iw_r, g4iw_i, g4err)
     implicit none
     ...
     call h5gopen_f(new_file_id, grpname, grp_id, err)
     call h5dopen_f(grp_id, "value", dset_id, err)
     call h5dopen_f(grp_id, "error", dset_err_id, err)
     
     call h5dread_f(dset_id, type_r_id, tmp_r, dims, err)
     call h5dread_f(dset_id, type_i_id, tmp_i, dims, err)
     call h5dread_f(dset_err_id, h5t_native_double, tmp_err, dims, err)

     tmp_r(:,:) = g4iw_r(:,:,iwb+1)+tmp_r(:,:)
     tmp_i(:,:) = g4iw_i(:,:,iwb+1)+tmp_i(:,:)
     tmp_err(:,:) = sqrt(g4err(:,:,iwb+1)**2+tmp_err(:,:)**2)
